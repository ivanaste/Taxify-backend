...
public class FindSuitableDriver {
    private final GetClosestDriverWithFilters getClosestUnoccupiedDriver;
	...
	public DriverResponse execute(RequestedRideRequest requestedRideRequest) throws IOException, InterruptedException, ExecutionException, StripeException {
		...
		Driver closestDriver = getClosestUnoccupiedDriver.execute(requestedRideRequest);
        ...
        if (checkoutRide.execute(newRide)) {
			...
			for (String email: requestedRideRequest.getPassengers().getRecipientsEmails()) {
				notifyPassengerOfChangedRideState.execute(email, NotificationType.RIDE_ACCEPTED);
			}
